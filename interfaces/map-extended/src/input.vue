<template>
	<div class="interface-map">
		<div :class="{ 'map-readonly': readonly }" class="map">
			<div :id="name" class="map-container" :style="{ height: style.height }">
				<!-- Map Renders Here -->
			</div>

			<div class="map-actions">
				<div v-if="options.address_to_code" class="address-input">
					<v-input v-model="placeName" v-bind:placeholder="contents.address_placeholder"></v-input>
					<button
						v-if="isInteractive"
						v-tooltip="contents.address_location"
				...
				<button
					v-if="isInteractive"
					v-tooltip="contents.my_location"
				...
				<button
					v-if="isInteractive"
					v-tooltip="contents.clear_location"
...
<script>
	...
	import './leaflet.css';
	import { debounce, forEach, get, size } from 'lodash';
	...
	export default {
		...
		computed: {
			...
			accentColor () {
				return getComputedStyle(document.documentElement)
					.getPropertyValue('--main-primary-color')
					.trim();
			},
			darkAccentColor () {
				return getComputedStyle(document.documentElement)
					.getPropertyValue('--main-primary-color-active')
					.trim();
			},
			contents () {
				return {
					address_placeholder: "Enter a valid address to geocode",
					address_location: "Set Address Location",
					my_location: "Set My Location",
					clear_location: "Clear Location",
					address_to_code_error: "No geocode found for this address...Please enter a valid address!",
					user_location_error: "There was an error while trying to get your location...Please try again!",
					user_location_error_blocked: "You've disabled the location access...Kindly enable it from the browser addressbar to auto detect your location.",
			...
			style () {
				...
				if (typeof this.options.height === "string" && /a-zA-Z/.test(this.options.height)) height = this.options.height;
				else if (this.options.height) height = `${ this.options.height }px`;
		...
		methods: {
			init () {
				let _latlng;
				/*
					If value is provided on initialization, map should be centered at lat/lng of value else it should center at provided default location.
				*/
				if (this.value) {
			...
			createMap (latlng) {
				...
				*/
				leaflet.tileLayer(this.options.theme, 
				{
			...
			*/
			...
			/*
				This function always will emit the value.
				...
				this.map.on('locationerror', result => {
					this.$events.emit('error', {
						notify:
							...
							*/
							result.code == 1
								? this.contents.user_location_error_blocked
								: this.contents.user_location_error,
						error: result
					});
			...
			*/
			markerSVG() {
				return `data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24"><path fill="${this.accentColor.replace(
					'#',
					'%23'
				)}" stroke-width="1" stroke="${this.accentColor.replace(
					'#',
					'%23'
			...
			*/ 
						...
						if (response.status === 200) {
							if (!response.data.features[0]) {
								this.$events.emit('warning', {
									notify: this.contents.address_to_code_error,
									error: 'result'
								});
							} 
							else {
								...
								let address = get(response.data, 'features.0.properties.address');
								...
								else this.setValue(coordinates);
			...
			/*
				Set the address components of additional fields - if applicable
			*/
			setAddress (address) {
				if (this.options.inputs) {
					forEach (this.options.inputs, (input, field) => {
						if (input) this.$emit('setfield', { field: input, value: get(address, field) });
					});
				}
			}
		}
