<template>
	<div :class="`interface-one-to-many interface-one-to-many-extended ${ editable ? 'editable' : 'readonly' }`">
		
		<v-notice v-if="relationshipSetup === false" color="warning" icon="warning">The relationship hasn't been configured correctly!</v-notice>

		<v-spinner v-else-if="initialValue === null"></v-spinner>
		...
		<template v-else>
				...
				<draggable
					...
					<div
						...
						<div
							...
							<v-icon v-if="!readonly" name="drag_handle" class="drag-handle"></v-icon>
						</div>
						<div
							v-for="field in visibleFields"
							:key="field.field"
			...
			<v-notice v-else>No Items have been selected!</v-notice>
...
<script>
	...
	export default {
		...
		computed: {
			editable () {
				return this.options.allow_create && this.options.allow_select;
			},
			/*
		...
		methods: {
			...
			*/
			...
			async getInitialValue () {
				...
				let filters = this.formatFilters(this.filters);
			...
			/*
				Change the sort position to the provided field. If the same field is changed, flip the sort order
			*/ 
			...
			saveEditItem () {
				...
				this.items = this.items.map(item => {
					if (item[this.relatedPrimaryKeyField] === primaryKey) {
						...
						/*
							Make sure we remove the many to one field that points to this o2m
							to prevent this nested item to be accidentally assigned to another parent
						*/ 
					...
					.map(after => {
						...
						if (before) {
							...
							/*
								...
								For a nested JSON record, we want to save the whole new state of the object, 
								instead of just the values that changed, seeing it will override the saved value 
								with a new Object only containing the changes.
...
<style lang="scss" scoped>
	.interface-one-to-many.interface-one-to-many-extended {
		.table {
