<template>
	<div class="interface-many-to-many interface-many-to-many-extended">
		...
		<template v-else>
			...
			<v-notice v-else color="gray-subdued" icon="info">No Item was Selected!</v-notice>
		...
		<portal v-if="editItem" to="modal">
			<v-modal
				:title="addNew ? 'Creating Item' : 'Editing Item'"
				:buttons="{
					save: {
						text: 'Save',
						color: 'accent'
				...
				const newValue = value
					.map(after => {
						...
						*/ 
						...
						if (before) {
							...
							if (Object.keys(delta).length > 0) {
								...
								/*
									Just in case there's an edit in the deep-nested recursive copy of the parent item delete it
								*/ 
						...
						*/ 
						...
						/*
							If the new (before case was handled above) item's primary key field is set, 
							then this is only the reference and we should only send the primary key field to the API. 
...
<style lang="scss">
	.interface-many-to-many.interface-many-to-many-extended {
		position: relative;
